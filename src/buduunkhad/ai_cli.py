...
def _context(config_path: Path, ai_config_path: Path | None = None):
    ...
    from buduunkhad.runtime_env import load_repository_env

    load_repository_env(config_path)
...
def prepare(
    ...
    try:
        ...
        directory, package = prepare_request_package(
            ...
            reasoning_mode=(
                project.ai.reasoning_mode.value if project.ai.reasoning_mode is not None else None
            ),
            text_verbosity=(
                project.ai.text_verbosity.value if project.ai.text_verbosity is not None else None
            ),
            store_response=project.ai.store_responses,
...
@phase03_ai_app.command("run-ai-first")
def phase03_run_ai_first(
    run_id: str = typer.Option(..., "--run-id", help="Exact sealed Phase 00-03/04 run ID."),
    ai_run_id: str | None = typer.Option(
        None,
        "--ai-run-id",
        help="Optional retry-attempt ID; a fresh UUIDv7 is generated by default.",
    ),
